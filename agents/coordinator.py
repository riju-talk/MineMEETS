...
class MeetingCoordinator:
    ...
    async def _process_meeting_content(
        self, meeting_data: Dict[str, Any], meeting_id: str
    ) -> Dict[str, Any]:
    ...
    async def _process_transcript(
        ...
        meeting_metadata = {
            ...
            "type": "transcript",
            "source_type": (
                "transcript_file" if "content_path" in meeting_data else "raw_transcript"
            ),
        }
        ...
        try:
            ...
            formatted_chunks = []
            for chunk in chunks:
                formatted_chunks.append({"text": chunk.page_content, "metadata": chunk.metadata})
            ...
            logger.warning(
                f"LangChain transcript processing failed, falling back to legacy method: {str(e)}"
            )
    ...
    async def _process_file(self, meeting_data: Dict[str, Any], meeting_id: str) -> Dict[str, Any]:
        ...
        if not validation["can_process"]:
            raise ValueError(
                f"File too large: {validation['file_size_mb']}MB. Maximum is {self.max_file_size_mb}MB."
            )
    ...
    async def _process_file_fallback(
        self, meeting_data: Dict[str, Any], meeting_id: str
    ) -> Dict[str, Any]:
    ...
    async def _process_audio(self, meeting_data: Dict[str, Any], meeting_id: str) -> Dict[str, Any]:
        ...
        audio_resp = await self.audio_agent.process(
            {"file_path": meeting_data["content_path"]}, context={"meeting_id": meeting_id}
        )
        ...
        chunks = audio_resp["content"].get("chunks") or await self._chunk_text_safe(
            raw_content, meeting_id
        )
    ...
    def _validate_file(self, file_path: str) -> None:
        ...
        if file_size > self.max_file_size_mb:
            raise ValueError(
                f"File too large: {file_size:.2f}MB. Maximum is {self.max_file_size_mb}MB."
            )
    ...
    async def _chunk_text_safe(
        self, text: str, meeting_id: str, chunk_size: int = None, chunk_overlap: int = None
    ) -> List[Dict]:
                ...
                if self._needs_internet_augmentation(answer_data.get("answer", "")):
                    ...
                    if internet_response["success"] and internet_response["content"].get("answer"):
                        ...
                        merged_response = self._merge_responses(
                            answer_data, internet_response["content"]
                        )
    ...
    def _create_error_response(
        self, meeting_id: str, error_message: str, error_type: str
    ) -> Dict[str, Any]:
