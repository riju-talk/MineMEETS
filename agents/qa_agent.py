...
Please provide any potentially relevant information from the meeting, or explain why the meeting doesn't cover this topic:"""
    ...
    async def _fallback_qa(
        ...
        try:
            ...
            if max_relevance < 0.6:  # Low relevance threshold
                ...
                prompt = self.qa_prompt_template.format(context=context_block, question=question)
    ...
    async def _smart_query(
        self, question: str, namespace: str = None, top_k: int = 10
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if keywords:
                for keyword in keywords[:3]:  # Use top 3 keywords
                    keyword_hits = self.pinecone_db.query_text(
                        keyword, namespace=namespace, top_k=3
                    )
                    all_hits.extend(keyword_hits)

            # Strategy 3: Broader context search for general questions
            if self._is_general_question(question):
                general_hits = self.pinecone_db.query_text(
                    "meeting discussion topics", namespace=namespace, top_k=5
                )
...
Please extract and share any information from the meeting that could be helpful:"""
    ...
    async def _handle_insufficient_context(self, question: str, namespace: str) -> Dict[str, Any]:
        ...
        return {"success": True, "content": {"answer": answer, "sources": []}}
    ...
    async def _fallback_overview(self, meeting_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            hits = self.pinecone_db.query_text(
                "meeting discussion conversation topics", namespace=meeting_id, top_k=20
            )
...
Structure your response to be informative and useful:"""
            ...
            return {"success": False, "content": f"Failed to generate meeting overview: {str(e)}"}
