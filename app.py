...
def ask_question(meeting_id: str, question: str) -> str:
    ...
    try:
        ...
        # Run async Q&A
        result = asyncio.run(coordinator.qa_agent.process(
            question,
        ...
        **Production MLOps Platform** for processing and querying meeting content across text, audio, and images.
        ...
        with gr.Tabs():
            ...
            with gr.Tab("📁 Upload & Process"):
                ...
                with gr.Row():
                    with gr.Column():
                        ...
                        text_file = gr.File(label="Upload Text File", file_types=[".txt", ".pdf", ".docx"])
                    ...
                    with gr.Column():
                        ...
                        audio_file = gr.File(label="Upload Audio File", file_types=[".mp3", ".wav", ".m4a", ".ogg"])
                        audio_btn = gr.Button("Process Audio", variant="primary")
                        audio_output = gr.Textbox(label="Processing Result", lines=5)
                        audio_btn.click(process_audio_file, inputs=[audio_file], outputs=[audio_output])
                    
                    with gr.Column():
                        gr.Markdown("#### 🖼️ Image Files (.png, .jpg, .jpeg)")
                        image_file = gr.File(label="Upload Image File", file_types=[".png", ".jpg", ".jpeg", ".webp"])
                        image_btn = gr.Button("Process Image", variant="primary")
                        image_output = gr.Textbox(label="Processing Result", lines=5)
                        image_btn.click(process_image_file, inputs=[image_file], outputs=[image_output])
            ...
            with gr.Tab("💬 Q&A"):
                ...
                ask_btn.click(
                    ask_question,
                    inputs=[meeting_id_input, question_input],
